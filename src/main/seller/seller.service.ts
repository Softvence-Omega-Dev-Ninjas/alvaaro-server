...
export class SellerService {
	...
	async createSeller(createSellerDto: CreateSellerDto, userId: string, userEmail: string) {
	...
	async findAll(filters: { verificationStatus?: "PENDING" | "VERIFIED" | "REJECTED"; subscriptionStatus?: "ACTIVE" | "INACTIVE"; search?: string }) {
		try {
			...
			const result = await this.prisma.seller.findMany({
				where: {
					...
					...(search && {
						OR: [{ user: { fullName: { contains: search, mode: "insensitive" } } }, { user: { email: { contains: search, mode: "insensitive" } } }]
					})
				},
				include: {
					user: true
	...
	async contactSeller(productId: string, contactSellerDto: ContactSellerDto) {
		...
		try {
			...
			await this.mail.sendMail(product.seller.user.email, `New inquiry for ${product.name}`, htmlContent)
			...
			return ApiResponse.success(result, "Inquiry sent successfully to the seller!")
		} catch (error) {
			if (error instanceof NotFoundException) {
				return ApiResponse.error("Product not found")
			}

			return ApiResponse.error("Failed to contact seller", error instanceof Error ? error.message : "Unknown error occurred")
		}
	}
	...
	async getDashboardStatistics(userId: string) {
		...
		try {
			const [totalViews, totalListings, totalEngagement, totalSaves] = await Promise.all([
			...
			return ApiResponse.success(result, "Seller stats retrieved successfully!")
		} catch (error) {
	...
	async getDashboardAnalysis(userId: string) {
		...
		const allMonths = new Set([...Object.keys(productCounts), ...Object.keys(inquiryCounts), ...Object.keys(saveCounts), ...Object.keys(viewsCounts)])
		...
		return ApiResponse.success(result, "Seller dashboard analytics retrieved successfully!")
	}
}
